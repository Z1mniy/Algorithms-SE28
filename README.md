...
Всегда очень удобно подставить рандомные числа и посмотреть сколько совершится операций. Для n = 10, k = 3 результатом будет 3. Очень напоминает логарифм, а так и есть. __Ответ: O(log от k по n)__
